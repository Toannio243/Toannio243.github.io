...
function parseInstruction (instruction) {
	switch (instruction[0]) {
		...
		case 'ir':
			...
			terminalOut("<p>Cambiando de habitacion a " + next_room_name + "</p>");
		...
		case 'coger':
			...
			game_data.rooms[current_room].items.forEach(function (item) {
				if (item == instruction[1]) {
					...
					if (item_num < 0) {
						console.log("Error al eliminar el item de la habitacion");
						return;
					}
...
function game (data) {
	...
	terminalOut("<p><strong>!Benvenido a ENTIerrame!</strong> El juego de terror de ultratumba.</p>");
